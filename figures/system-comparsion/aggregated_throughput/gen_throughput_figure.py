...
for i, workload in enumerate(workloads):
    ...
    legend_labels = []
    for idx, subcat in enumerate(labels):
            if 'Super' in subcat:
                legend_labels.append(r'$\bf{SUPER}$')
                # subcat[i] = r'$\bf{SUPER}$'
            else:
                legend_labels.append(subcat)
    axs[i].legend(legend_labels,fontsize=10, loc='upper left')
...
plt.savefig('figures/system-comparsion/aggregated_throughput/throughput.png')
plt.show()
