...
def process_reprot(folder_path, is_language):
    metrics = OrderedDict({
         "num_jobs": 0,
        #  "elapsed_time(s)": 0,
         "total_bathces":0,
...
# def process_reprot(folder_path):
#     metrics = OrderedDict({
#          "num_jobs": 0,
#          "elapsed_time(s)": 0,
#          "total_bathces":0,
...
#     metrics["data_time(s)"] = abs(metrics["data_time(s)"] - metrics["transform_time(s)"])
...
if __name__ == "__main__":
    ...
    for subfolder in subfolders:
        if 'openwebtext' in folder_path:
            csv_data = process_reprot(subfolder,is_language=True)
        else:
            csv_data = process_reprot(subfolder,is_language=False)
