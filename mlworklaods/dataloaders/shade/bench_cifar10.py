...
from mlworklaods.dataloaders.shade.shadedataset import ShadeDataset, ShadeValDataset
from mlworklaods.dataloaders.shade.shadesampler import ShadeSampler
...
def main():
	...
	parser.add_argument('--train_paths', nargs="+", help="Paths to the train set", default=['data/cifar-10/train'])
	parser.add_argument('--master_address', type=str, default='', help='The address of master node to connect into')
	parser.add_argument('--master_socket', type=str, default='', help='The NCCL master socket to connect into')
	...
	parser.add_argument('--cache_training_data', default=True, action='store_true',
	...
	parser.add_argument('--network', default='resnet18', type=str,help='''The neural network we use for the training''')
	parser.add_argument('--batch_size', default=256, type=int, help="Batch size used for training and validation")
	parser.add_argument('--master_port', default=12345, type=int,help='The port on which master is listening')
	...
	parser.add_argument('--port_num', nargs='?', default='6379', const='6379', help='port that redis cluster is listening to')
	parser.add_argument('-cn', '--cache_nodes', default=1, type=int, help='number of nodes cache is distributed across')
...
def train(gpu, args):
	...
	train_sampler = ShadeSampler(train_dataset,
	...
	for epoch in range(args.epochs):
		...
		while batch_counter < len(train_loader):
			...
			# Assuming detailed_log_per_epoch is your existing DataFrame
			new_row = {
			...
			detailed_log_per_epoch = pandas.concat([detailed_log_per_epoch, pandas.DataFrame(new_row, index=[0])], ignore_index=True)

			# detailed_log_per_epoch = detailed_log_per_epoch.append(new_row_df, ignore_index=True)
