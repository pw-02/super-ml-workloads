...
class SUPERDataset(IterableDataset):
        ...
        while self.index < iter_len:
            if prepared_batches:
                ...
                if next_batch_size == self.batch_size:
                    ...
                    torch_imgs, torch_labels, batch_id = self.__getitem__(next_batch.batch_id, next_batch.indicies, next_batch.is_cached)

                    while torch_imgs.size(0) != self.batch_size and self.index < iter_len:
                        new_batch = self._get_next_super_batch()
    ...
    def fetch_from_cache(self, batch_id, max_attempts = 1):     
        ...
        try:
            ...
            logger.error(f"Error fetching batch '{batch_id}' from cache: {e}")
            cached_data = None
        return cached_data
