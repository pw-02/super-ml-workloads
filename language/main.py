...
from gpt.datasets import *
from gpt.samplers import *
from gpt.training import *
from gpt.logger import SUPERLogger
import tiktoken
# support running without installing as a package
wd = Path(__file__).parent.parent.resolve()
sys.path.append(str('language/gpt'))

from gpt.model import GPT, Config

# from gpt.model_config import Config
...
def initialize_model(fabric: Fabric, model_name: str) -> nn.Module:
...
def initialize_dataloader(job_id, fabric: Fabric, num_workers, dataloader_backend, data_dir, shuffle, sampler_seed,
                          ...
                          cache_port):
    dataset = initialize_dataset(job_id=job_id, dataloader_backend=dataloader_backend,batch_size=batch_size, data_dir=data_dir,
...
def initialize_sampler(fabric:Fabric, job_id, dataset, dataloader_backend, shuffle, sampler_seed, batch_size, drop_last,
    ...
    if dataloader_backend == "super":
        return SUPERSampler(job_id=job_id, data_source=dataset, batch_size=1, drop_last=drop_last,
                            shuffle=shuffle, seed=sampler_seed,
                            super_prefetch_lookahead=super_prefetch_lookahead,
                            super_address=super_address)

    elif dataloader_backend == "pytorch-batch":
        return PytorchBatchSampler(data_source=dataset, batch_size=1, drop_last=drop_last, shuffle=shuffle,
                                   seed=sampler_seed)
...
def initialize_dataset(job_id, dataloader_backend: str, batch_size: int, data_dir: str,
                       cache_host=None, cache_port=None):
    if dataloader_backend == "super":
        return SUPERDataset(job_id=job_id, data_dir=data_dir, batch_size=batch_size, cache_host=cache_host,
                            cache_port=cache_port)
    elif dataloader_backend == "pytorch-batch":
        return PytorchBatchDataset(job_id=job_id, data_dir=data_dir, batch_size=batch_size)
    elif dataloader_backend == "pytorch-vanilla":
        return PytorchVanillaDataset(job_id=job_id, data_dir=data_dir, batch_size=batch_size)
