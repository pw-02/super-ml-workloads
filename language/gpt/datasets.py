...
# Create S3TextDataset and DataLoader with RandomSampler
s3_dataset = S3TextDataset(s3_bucket, s3_prefix, block_size)
s3_sampler = RandomSampler(s3_dataset)
s3_data_loader = DataLoader(s3_dataset, batch_size=batch_size, sampler=s3_sampler)
...
class BaseDataset(Dataset):
    def __init__(self, job_id, data_dir: str, batch_size: int, block_size:int):
        self.job_id = job_id
    ...
    def generate_id(self):
        import hashlib
        dataset_info = { "data_dir": self.data_dir}
    ...
    def __getitem__(self, next_batch):
        idx, batch_id = next_batch

        if self.use_s3:
            text, fetch_time = self.fetch_data_s3(idx, batch_id)
        else:
            text, fetch_time = self.fetch_data_local(idx, batch_id)

        data, transform_time = self.tokensize(text)
        ...
        return x,y, batch_id, False, fetch_time, transform_time

    @timer_decorator
    def tokensize(self, example):
    ...
    def is_allowed_file(self, filename: str):
        return any(filename.endswith(extension) for extension in self.allowed_extensions)
    ...
    def _classify_samples_local(self, data_dir) -> Dict[str, List[str]]:
        ...
        if index_file.exists():
            ...
            for dirpath, dirnames, filenames in os.walk(data_dir):
                for filename in filter(self.is_allowed_file, filenames):
        ...
        for page in pages:
            for blob in page['Contents']:
                ...
                if stripped_path == blob_path:
                    continue
                if not self.is_allowed_file(blob_path):
                    continue
...
class PytorchVanillaDataset(BaseDataset):
    ...
    def __init__(self, job_id, data_dir: str, batch_size: int, block_size:int):
        super(PytorchVanillaDataset, self).__init__(job_id, data_dir, batch_size, block_size)
...
class PytorchBatchDataset(BaseDataset):
      ...
      def __init__(self, job_id, data_dir: str, batch_size: int, block_size:int):
        super(PytorchBatchDataset, self).__init__(job_id, data_dir, batch_size, block_size)
 

class SUPERDataset(BaseDataset):
    def __init__(self, job_id, data_dir: str, batch_size: int,block_size:int, cache_host, cache_port):
        super(SUPERDataset, self).__init__(job_id, data_dir, batch_size, block_size)
