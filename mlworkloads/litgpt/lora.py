...
from mlworkloads.language.litgpt.openwebtext import OpenWebText
...
def fit(
    ...
    fabric.print(
        f"The longest sequence length in the train data is { model.max_seq_length}, the model's maximum sequence length is"
...
if __name__ == "__main__":
    setup(checkpoint_dir=Path("mlworkloads\litgpt\checkpoints\EleutherAI\pythia-14m"), 
          devices=1, 
          num_nodes=1, 
          eval=EvalArgs(interval=100, max_new_tokens=100, max_iters=100, initial_validation=False, final_validation=False),
