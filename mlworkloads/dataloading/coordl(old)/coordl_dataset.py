...
logging.basicConfig(
    level=logging.DEBUG,  # Log messages of this level or higher
    format='%(asctime)s - %(levelname)s - %(message)s',  # Log format
)

logger = logging.getLogger(__name__)  # Use __name__ to name the logger after the module
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, float, float]:
        ...
        if next_minibatch  is not None and (isinstance(next_minibatch , bytes) or isinstance(next_minibatch , str)):
            ...
            if self.use_cache and self.cache_transformations:
                try:
                    self._initialize_cache_client()
                    cached_after_fetch = self.cache_minibatch_with_retries(batch_id, batch_id)
                except Exception as e:
                    print(f"Error saving to cache: {e}, batch_id: {batch_id}")
        
        return (batch_id, batch_indices)
        # Calculate data loading time excluding transformation time
...
class CoorDLBatchSampler(BatchSampler):
    def __init__(self, data_source: Sized, 
        ...
        self.cache_client = redis.StrictRedis(host="127.0.0.1",port=6379)
        ...
        self.worker_batches = self._distribute_work(self.batches, self.world_size)

        logger.info(f"Worker {self.worker_id} assigned {len(self.worker_batches)} batches")

    
    def _distribute_work(self,items, num_workers):
        total_items = len(items)
        items_per_worker = total_items // num_workers
        remaining_items = total_items % num_workers
        # Distribute the "shards" of the dataset across workers   
        start_index = self.worker_id * items_per_worker
        end_index = start_index + items_per_worker + (1 if self.worker_id < remaining_items else 0)
        # Assign batches from the assigned shard to the worker
        assigned_batches = items[start_index:end_index]
        # Remove he assigned batches from the original batch list
    ...
    def _create_batches(self) -> List[List[int]]:
        ...
        for idx in self.sampler:
            ...
            if len(batch_indices) == self.batch_size:
                ...
                batch_list.append((batch_id, batch_indices))
                batch_indices = []
    ...
    def __iter__(self) -> Iterator[Tuple[str, List[int]]]:
        ...
        processed_batches = set()  # Keep track of the batches already processed
        total_batches = len(self.batches) + len(self.worker_batches)
        # Loop until all batches are processed
        while len(processed_batches) < total_batches:
            # Check for batches already in the cache
            cached_batches = []
            redis_keys = list(self.cache_client.keys('*'))
            for batch in self.batches:
                if batch[0].encode() in redis_keys and batch[0] not in processed_batches:
                    cached_batches.append(batch)

            # If cached batches exist, yield them first
            if cached_batches:
                # print(f"Found {len(cached_batches)} cached batches")
                for batch in cached_batches:
                    yield batch
                    processed_batches.add(batch[0])  # Mark this batch as processed

            # If no cached batches found, fall back to worker batches
            elif self.worker_batches:
                # Yield from worker batches if no cached batches
                batch = self.worker_batches.pop(0)
                yield batch
                processed_batches.add(batch[0])  # Mark this batch as processed
            else:
                # Wait for a short period before checking again
                # print("No batches in cache or worker batches left. Waiting...")
                time.sleep(1)  # Wait for a second before checking again

        # Regenerate batches after finishing an epoch (optional)
        self.batches = self._create_batches()
...
class Job:
    def __init__(self, job_id, total_jobs, job_speed):
        self.job_id = job_id
        self.world_size = total_jobs # Shared cache (staging area)
    ...
    def process_minibatches(self):
        ...
        # dataset_size = 1
        cooordl_dataset = CoorDLBatchedDataset(s3_data_dir="s3://sdl-cifar10/test/", 
                                               transform=transform,
                                               cache_address='27.0.0.1:6379',
                                               cache_transformations=True)

        # # Example usage of BatchSamplerWithID with shuffling
        batch_sampler_with_id = CoorDLBatchSampler(data_source=cooordl_dataset, 
                                                batch_size=1000, 
                                                drop_last=False, 
                                                shuffle=False, 
                                                seed=42,
        ...
        for batch_idx, (batch) in enumerate(batch_sampler_with_id):
            logger.info(f"Job {self.job_id} processed {batch[0]}, Total batches processed: {batch_idx +1}")
            time.sleep(self.job_speed)
            batch_sampler_with_id.cache_client.set(str(batch[0]), str(batch[1]))
        # for batch_idx, (batch) in enumerate(dataloader):
        #     print(f"Job {self.job_id}: {batch_idx +1}-Batch ID: {batch[0]}, Batch Indices: {len(batch[1])}")
...
if __name__ == "__main__":
    ...
    cache_client = redis.StrictRedis(host="127.0.0.1",port=6379)
    cache_client.flushdb()

    jobs = []
    job_speeds = [0.001, 0.002, 0.003, 0.004, 0.005]
    #job_speeds = [0.001, 0.005]
    for idx, (speed) in enumerate(job_speeds):
        job = Job(job_id=idx, total_jobs=len(job_speeds), job_speed=speed)
        jobs.append(job)
    ...
    # for batch_idx, (batch) in enumerate(batch_sampler_with_id):
    #     print(f"Batch ID: {batch[0]}, Batch Indices: {len(batch[1])}")
