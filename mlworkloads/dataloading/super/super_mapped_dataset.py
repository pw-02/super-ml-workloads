...
class SUPERMappedDataset(Dataset):
    def __init__(self, s3_data_dir: str, 
                 ...
                 cache_address= None, 
                 simulate_mode=False, 
                 simulate_time_for_cache_miss=0.25, 
                 simulate_time_for_cache_hit=0.045,
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, float, float]:
        ...
        if is_cached and self.use_cache: #we rely on is cached flag to determine if we should fetch from cache or not status to avoid cache misses  
            next_minibatch = self.get_cached_minibatch_with_retries(batch_id, max_retries=0.3, retry_interval=0.075)
        # else:
        ...
        if next_minibatch  is not None and (isinstance(next_minibatch , bytes) or isinstance(next_minibatch , str)):
            ...
            batch_data, batch_labels = self._bytes_to_torch_batch(next_minibatch)
    ...
    def get_cached_minibatch_with_retries(self, batch_id, max_retries=3, retry_interval=0.1):
        ...
        while attempt < max_retries:
            ...
            attempt += 1
            time.sleep(retry_interval)  # Sleep for 1 second before retrying
