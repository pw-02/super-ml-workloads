...
if __name__ == "__main__":
    setup(model_name="pythia-14m",
          model_config=None,
            out_dir=Path("out/pretrain"),
            precision="16-true",
            initial_checkpoint_dir=None,
            resume=False,
            data=OpenWebText(data_path='s3://litdatapreprocessed5mbowt', num_workers=0),
            train=TrainArgs(max_seq_length=512, max_tokens=1000000,
                            max_norm=1.0, save_interval=1000, log_interval=1, global_batch_size=1, micro_batch_size=1, min_lr=4e-5, lr_warmup_steps=1, tie_embeddings=False),
            eval=EvalArgs(interval=1000, max_iters=100,final_validation=False, initial_validation=False),
            optimizer="AdamW",
            devices=1,
            num_nodes=1,
            tokenizer_dir=None,
