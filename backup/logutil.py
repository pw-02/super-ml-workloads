...
class BaseMetrics:
    def __init__(self):
        ...
        self.loss = AverageMeter("Loss", ":.4e")
...
class EpochMetrics(BaseMetrics):
    def __init__(self):
        ...
        self.epcoh_top1 = AverageMeter('Acc@1', ':6.2f')
        self.epoc_top5 = AverageMeter('Acc@5', ':6.2f')

class ExperimentLogger():
    def __init__(self, fabric:Fabric, log_dir:str, flush_freq:int, print_freq:int):
        self.log_freq = print_freq
        ...
        self.gobal_step_count = 0
        self.total_samples = 0
        self.experiment_writer = ExperimentWriter(log_dir=log_dir, rank=self.fabric.local_rank)

    def save_train_batch_metrics(self,epoch,step,num_sampels,batch_time,data_time,compute_time,loss, cahce_hit = False):
          metrics = OrderedDict({
                "device": self.fabric.local_rank,
                "epoch": epoch,
                "epoch_step": step,
                "global_step": self.gobal_step_count,
    ...
    def loop_started(self, total_batches:int, is_train_loop:bool = True):
        dataset_type = 'train' if is_train_loop else 'val'
        self.epoch_metrics[dataset_type].total_batches = total_batches
        
    
    def record_iteration_metrics(self,epoch,step,global_step,num_sampels,iteration_time, data_time, compute_time, loss, train = True, cache_hit:bool = False):
       
        dataset_type = 'train' if train else 'val'
        self.total_samples += num_sampels
        ...
        self.iteration_metrics.loss.update(loss)
        self.epoch_metrics[dataset_type].num_samples.update(num_sampels)
        if cache_hit:
            self.iteration_metrics.cache_hits +=1
        # self.iteration_metrics.top1.update(top1)
        # self.iteration_metrics.top5.update(top5)
        # self.iteration_metrics.cpu_util.update(cpu_util)
        # self.iteration_metrics.cpu_util.update(gpu_util)
        display_progress = self.should_display(step)
        flush_to_file = self.should_flush(step)
                 
        if display_progress or flush_to_file:
            iteration_metrics_dict = OrderedDict({
                ...
                # "data_fetch": self.iteration_metrics.data_fetch_time.val,
                # "data_transform": self.iteration_metrics.data_transform_time.val,
                ...
                "loss": self.iteration_metrics.loss.val,
                # "top1": self.iteration_metrics.top1.val,
                # "top5": self.iteration_metrics.top5.val,
                "cache_hits": self.iteration_metrics.cache_hits
            })

            if display_progress:
                display_info = {"epoch": epoch, "step": f"{step}/{self.epoch_metrics[dataset_type].total_batches}"}
                temp_dict = {key: iteration_metrics_dict[key] for key in ["total_time", "data_time", "compute_time", "cache_hit"] if key in iteration_metrics_dict}
                display_info.update(temp_dict)
                self.fabric.print(", ".join(f"{key}: {round(float(value), 4)}" if isinstance(value, float) else f"{key}: {value}" for key, value in display_info.items()))
            if flush_to_file:
                self.log_metrics(metrics = iteration_metrics_dict,prefix=f'{dataset_type}.iteration',step=global_step,force_save = True)
        
    def epoch_end(self,epoch, train = True):

        dataset_type = 'train' if train else 'val'
        # total_batches = self.iteration_metrics.iteration_time.count
        epoch_time = self.iteration_metrics.iteration_time.sum
        # data_time = self.iteration_metrics.data_time.sum  
        # # data_fetch_time = self.iteration_metrics.data_fetch_time.sum
        # # data_transform_time = self.iteration_metrics.data_transform_time.sum
        # compute_time = self.iteration_metrics.compute_time.sum
        ...
        # self.epoch_metrics[dataset_type].data_fetch_time.update(data_fetch_time)
        # self.epoch_metrics[dataset_type].data_transform_time.update(data_transform_time)
        self.epoch_metrics[dataset_type].compute_time.update(self.iteration_metrics.compute_time.sum)
        self.epoch_metrics[dataset_type].loss.update(self.iteration_metrics.loss.avg)
        ...
        self.epoch_metrics[dataset_type].cache_hits.update(self.iteration_metrics.cache_hits)

        epoch_metrics = OrderedDict(
            {
                "device": self.fabric.local_rank,
                "epoch": epoch,
                "num_samples": self.epoch_metrics[dataset_type].num_samples.sum,
                "num_batches": self.epoch_metrics[dataset_type].total_batches.val,
                "total_time": epoch_time,
                "data_time": self.epoch_metrics[dataset_type].data_time.val,
                # "data_fetch": self.epoch_metrics[dataset_type].data_fetch_time.val,
                # "data_transform": self.epoch_metrics[dataset_type].data_transform_time.val,
        ...
        self.log_metrics(metrics=epoch_metrics,  prefix='train.epoch' if train else 'val.epoch', force_save = True)
    ...
    def log_metrics( self, metrics: Dict[str, Union[Tensor, float]],prefix:str, step:int = None, force_save:bool = False) -> None:
        ...
        elif  step is not None and (step + 1) % self.flush_freq == 0:
            self.experiment_writer.save()
    ...
    def should_flush(self, current_step):
        if (current_step + 1) % self.flush_freq == 0:
            return True
        else:
            return False
