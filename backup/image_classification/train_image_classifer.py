...
import image_classifer_utils as utils
from torch.utils.data import DataLoader
import torchvision
from torch_datasets.s3_imgae_mapped_dataset import S3MappedImageDataset
from torch.utils.data import SequentialSampler, RandomSampler  
from img_classifer_logger import ImageClassifierLogger, AverageMeter, ProgressMeter, Summary
import os
from image_clssifer_args import TrainArgs, IOArgs
...
def main(fabric: Fabric, seed: int, config: DictConfig, train: TrainArgs, io: IOArgs,) -> None:        
        ...
        logger = ImageClassifierLogger(fabric, io.log_dir, io.log_interval)
...
def fit(fabric: Fabric, model:nn.Module, optmizer, train_dataloader:DataLoader, val_dataloader:DataLoader, logger:ImageClassifierLogger, train_args:TrainArgs):
    best_acc1_train = 0
    best_acc5_train = 0
    best_acc1_eval = 0
    best_acc5_eval = 0
...
def val_loop(fabric: Fabric,epoch:int,model:nn.Module, val_dataloader:DataLoader,max_iters:int,logger:ImageClassifierLogger):
        total_samples  = 0
...
def train_loop(fabric: Fabric, epoch:int,model:nn.Module, optimizer, train_dataloader:DataLoader, max_iters:int, logger:ImageClassifierLogger):
        total_samples = 0
